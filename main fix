...
// 객체 탐지
void detectObj(const Mat& bin, Point& mainPoint, Mat& outputImg, Mat& stats, Mat& centers) {
    Mat label;
    // bin: 이진화된 이미지, label: 각 객체를 구분한 라벨 이미지
    // stats: 각 객체의 통계(면적, 위치 등), centers: 각 객체의 중심 좌표
    int numObjects = connectedComponentsWithStats(bin, label, stats, centers); // 객체 개수
    ...
    for (int i = 1; i < numObjects; i++) {
        int area = stats.at<int>(i, CC_STAT_AREA);  // 객체의 면적
        if (area > 120) {  // 면적이 120보다 큰 객체만 탐지
            Point center(cvRound(centers.at<double>(i, 0)), cvRound(centers.at<double>(i, 1)));  // 객체 중심 좌표
    ...
    if (closestObj >= 0) {
        mainPoint = Point(cvRound(centers.at<double>(closestObj, 0)),
            cvRound(centers.at<double>(closestObj, 1)));
    }
}

// 객체 표시
void markObj(const Mat& stats, const Mat& centers, const Point& mainPoint, Mat& outputImg) {
    ...
    for (int i = 1; i < stats.rows; i++) {
        ...
        if (area > MIN_OBJECT_AREA) {
            // 객체 중심 좌표 계산
            Point center(cvRound(centers.at<double>(i, 0)), cvRound(centers.at<double>(i, 1)));
...
int main() {
    ...
    Point mainPoint;  // mainPoint는 화면의 목표 지점
    Mat frame, gray, bin, outputImg, stats, centers;
    int Error;
    ...
    while (true) {
        ...
        detectObj(bin, mainPoint, outputImg, stats, centers);  // 객체 탐지
        markObj(stats, centers, mainPoint, outputImg);  // 객체에 표시하기
